...
class GroupDependencyJsonAdapter(
    ...
    private val listOfGroupChildModuleAdapter: JsonAdapter<List<GroupChildModule>> =
        moshi.adapter(
            Types.newParameterizedType(List::class.java, GroupChildModule::class.java),
            emptySet(), "modules"
        )
    ...
    override fun fromJson(reader: JsonReader): GroupDependency {
        ...
        while (reader.hasNext()) {
            when (reader.selectName(options)) {
                ...
                1 -> version = stringAdapter.fromJson(reader) ?: throw Util.unexpectedNull(
                    "version",
                    "version", reader
                )
                2 -> link = nullableStringAdapter.fromJson(reader)
                3 -> group = stringAdapter.fromJson(reader) ?: throw Util.unexpectedNull(
                    "group", "group",
                    reader
                )
